...
$now= date("H:i:s");
$aktifkan=mysqli_query($koneksi,"update jadwal set aktif=1 where idh=$id_hari[idh] AND jam_mulai<= '$now' AND jam_selesai>= '$now'");
$nonaktifkan=mysqli_query($koneksi,"update jadwal set aktif=0 where idh<>$id_hari[idh]");
$nonaktifkan2=mysqli_query($koneksi,"update jadwal set aktif=0 where idh=$id_hari[idh] AND jam_mulai>= '$now' OR jam_selesai<= '$now'");
